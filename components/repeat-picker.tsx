...
function generateRRuleString(config: RepeatConfig, timezone: string): string {
  ...
  const byweekday =
    config.frequency === "weekly" && config.daysOfWeek
      ? config.daysOfWeek.map((d) => dayToRRuleWeekday[d])
      : undefined;
...
export const RepeatPicker: React.FC<RepeatPickerProps> = ({
  ...
  const handleFrequencyChange = (
    ...
    setConfig((prev) => ({
      ...
      frequency,
      daysOfWeek:
        frequency === "weekly" ? prev.daysOfWeek || [1, 3, 5] : undefined,
    }));
  };
